...
async def debug_collection_date():
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        async with session.get(url) as response:
            ...
            if items:
                ...
                print(f"Attribution: {item.get('cardAttributionPrepared')}")
                # Depth check
                if item.get('cardAttributionPrepared'):
                    print(f"Date: {item['cardAttributionPrepared'].get('publishedDate')}")

if __name__ == "__main__":
    asyncio.run(debug_collection_date())
