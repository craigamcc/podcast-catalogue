...
.PHONY: help export-sota export-daisy run-http diag
...
export-sota:
	...
	python3 -m podcast_catalogue.cli --input $(RAW_DATA) --format json --tier 0 --output "$(SOTA_APP_DIR)/catalogue.json"
	@echo "✅ SOTA export complete. Slim discovery catalogue refreshed."

export-daisy:
	@echo "🌼 Syncing Tier 1 Intelligence to Daisy..."
	python3 -m podcast_catalogue.cli --input $(RAW_DATA) --format json --tier 1 --output "$(DAISY_APP_DIR)/intelligence.json"
	@echo "✅ Daisy export complete. Intelligence graph refreshed."

run-http:
	@echo "🌉 Starting PRISM HTTP Bridge..."
	uvicorn podcast_catalogue.prism_http:app --reload --port 8000

diag:
	python3 -m podcast_catalogue.cli --check-env
